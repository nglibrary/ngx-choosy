...
describe('ChoosyResultsComponent', () => {
  ...
  describe('what if', () => {
    it('doesnt have config input? - should inherit default config', () => {
      ...
      comp.ngOnInit();
      expect(comp.config).toEqual(configService.getDefaultConfig());
    });
  ...
  describe('ngOnInit', () => {
    ...
    it('makes expected calls', () => {
      ...
      spyOn(comp, 'expose');
      spyOn(choosyConfigServiceStub, 'getConfig');
      comp.ngOnInit();
      expect(comp.expose).toHaveBeenCalled();
      expect(choosyConfigServiceStub.getConfig).toHaveBeenCalled();
    });
