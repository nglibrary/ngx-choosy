...
import { Option, Options, Event } from '../../models';
...
export class ListService {
  optionsSub: BehaviorSubject<Options> = new BehaviorSubject([]);
  settings: any = {
    type: 'select'
  };
  selectedOptionsBucket: Options = [];
  ...
  private latestFilteredOptions: Options = [];
  constructor(private searchService: SearchService) {}

  selectOption(option: Option): void {
    const opts = this.optionsSub.getValue().map((o: Option) => {
  ...
  removeOption(fn: (option: any) => boolean): void {
    let removedOpt = null;
    const opts = this.optionsSub.getValue().filter((o: Option) => {
  ...
  setOptions(options: Options, settings: any): void {
  ...
  mapMetaData(options: any[]): Options {
  ...
  private metaData(option): Option {
    return {
